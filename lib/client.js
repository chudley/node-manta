...
MantaClient.prototype.createWriteStream = function createWriteStream(p, opts) {
    ...
    doCreateWriteStream(this, log, options, stream, cb, opts.mkdirs);

    return (stream);
};

function doCreateWriteStream(self, log, options, stream, cb, allowretry) {
    self.signRequest({
        headers: options.headers
    }, function onSignRequest(err) {
        if (err) {
        ...
        self.client.put(options, onRequestCallback({
            ...
            onResult: onResultCallback({
                cb: function (puterr, res) {
                    ...
                    log.debug('createWriteStream with mkdirp: mkdirp');
                    var parent = path.posix.dirname(path.posix.normalize(
                        options._original_path));
                    self.mkdirp(parent, function onMkdirp(mkdirperr) {
                        if (mkdirperr) {
                            log.debug(mkdirperr,
                                'createWriteStream with mkdirp: error');
                            cb(mkdirperr);
                            return;
                        }

                        log.debug('createWriteStream with mkdirp: mkdirp done');
                        doCreateWriteStream(self, log, options,
                            stream, cb, false);
                    });
                },
                log: log,
